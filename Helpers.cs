...
namespace CalradiaExpandedKingdoms.Helpers
{
  public static class CEKHelpers
  {
    ...
    public static bool IsInCultureGroup(string id, string referenceId)
    {
      ...
      if (referenceId == "empire" || referenceId == "apolssaly" || referenceId == "republic" || referenceId == "ariorum")
        return id == "empire" || id == "apolssaly" || id == "republic" || id == "ariorum";
    ...
    public static Kingdom GetKingdomByID(string ID) => Kingdom.All.FirstOrDefault<Kingdom>((Func<Kingdom, bool>) (x => x.StringId == ID));

    public static Hero GetClanLeaderByID(string id) => Clan.All.FirstOrDefault<Clan>((Func<Clan, bool>) (x => x.StringId == id)).Leader;

    public static Clan GetClanByID(string id) => Clan.All.FirstOrDefault<Clan>((Func<Clan, bool>) (x => x.StringId == id));

    public static CharacterObject GetNPCByID(string id) => id == null ? (CharacterObject) null : CharacterObject.All.FirstOrDefault<CharacterObject>((Func<CharacterObject, bool>) (x => x.StringId == id));

    public static CultureObject GetCultureObjectByID(string id) => MBObjectManager.Instance.GetObjectTypeList<CultureObject>().FirstOrDefault<CultureObject>((Func<CultureObject, bool>) (x => x.StringId == id));
